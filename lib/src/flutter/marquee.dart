...
class VxMarqueeState extends State<VxMarquee>
  ...
  void startTimer() {
    final double widgetWidth = _key.currentContext!
        .findRenderObject()!
        .paintBounds
        .size
        .width;
    final double widgetHeight = _key.currentContext!
        .findRenderObject()!
        .paintBounds
        .size
        .height;
    ...
    timer = Timer.periodic(Duration(milliseconds: duration), (timer) {
      ...
      if (pixels + _moveDistance >= maxScrollExtent) {
        if (widget.scrollAxis == Axis.horizontal) {
          ...
          position =
              (maxScrollExtent - blankWidth! - widgetWidth) / 2 +
              pixels -
              maxScrollExtent;
        } else {
          position =
              (maxScrollExtent - blankHeight! - widgetHeight) / 2 +
              pixels -
              maxScrollExtent;
        }
